...
<head>
    ...
    <link rel="apple-touch-icon"
          href="{{ ($AMSSettings) && ($AMSSettings->favicon!='') ?  Storage::disk('public')->url(e($AMSSettings->logo)) :  config('app.url').'/img/neepco-logo-bug.png' }}">
    <link rel="apple-touch-startup-image"
          href="{{ ($AMSSettings) && ($AMSSettings->favicon!='') ?  Storage::disk('public')->url(e($AMSSettings->logo)) :  config('app.url').'/img/neepco-logo-bug.png' }}">
    <link rel="shortcut icon" type="image/ico"
          href="{{ ($AMSSettings) && ($AMSSettings->favicon!='') ?  Storage::disk('public')->url(e($AMSSettings->favicon)) : config('app.url').'/favicon.ico' }}">
    ...
    @if (($AMSSettings) && ($AMSSettings->allow_user_skin==1) && Auth::check() && Auth::user()->present()->skin != '')
        <link rel="stylesheet" href="{{ url(mix('css/dist/skins/skin-'.Auth::user()->present()->skin.'.min.css')) }}">
    @else
        <link rel="stylesheet"
              href="{{ url(mix('css/dist/skins/skin-'.($AMSSettings->skin!='' ? $AMSSettings->skin : 'blue').'.css')) }}">
    @endif
    {{-- page level css --}}
    @stack('css')
    ...
    @if (($AMSSettings) && ($AMSSettings->header_color!=''))
        <style nonce="{{ csrf_token() }}">
            ...
            .skin-{{ $AMSSettings->skin!='' ? $AMSSettings->skin : 'blue' }} .sidebar-menu > li:hover > a, .skin-{{ $AMSSettings->skin!='' ? $AMSSettings->skin : 'blue' }} .sidebar-menu > li.active > a {
...
@if (($AMSSettings) && ($AMSSettings->allow_user_skin==1) && Auth::check() && Auth::user()->present()->skin != '')
    <body class="sidebar-mini skin-{{ $AMSSettings->skin!='' ? Auth::user()->present()->skin : 'blue' }} {{ (session('menu_state')!='open') ? 'sidebar-mini sidebar-collapse' : ''  }}">
    @else
        <body class="sidebar-mini skin-{{ $AMSSettings->skin!='' ? $AMSSettings->skin : 'blue' }} {{ (session('menu_state')!='open') ? 'sidebar-mini sidebar-collapse' : ''  }}">
        @endif
        ...
        <div class="wrapper">
                                ...
                                <a class="logo navbar-brand no-hover" href="{{ config('app.url') }}">
                                    @if ($AMSSettings->logo!='')
                                        <img class="navbar-brand-img"
                                             src="{{ Storage::disk('public')->url($AMSSettings->logo) }}"
                                             alt="{{ $AMSSettings->site_name }} logo">
                                    @endif
                                    {{ $AMSSettings->site_name }}
                                </a>
                                ...
                                <a class="logo navbar-brand no-hover" href="{{ config('app.url') }}">
                                    @if ($AMSSettings->logo!='')
                                        <img class="navbar-brand-img"
                                             ...
                                             alt="{{ $AMSSettings->site_name }} logo">
                                    @endif
                                    <span class="sr-only">{{ $AMSSettings->site_name }}</span>
                                </a>
                            @else
                                <a class="logo navbar-brand no-hover" href="{{ config('app.url') }}">
                                    {{ $AMSSettings->site_name }}
                        ...
                        <ul class="nav navbar-nav">
                            @can('index', \App\Models\Asset::class)
                                <li aria-hidden="true"{!! (Request::is('hardware*') ? ' class="active"' : '') !!}>
                                    <a href="{{ url('hardware') }}" {{$AMSSettings->shortcuts_enabled == 1 ? "accesskey=1" : ''}} tabindex="-1" data-tooltip="true" data-placement="bottom" data-title="{{ trans('general.assets') }}">
                            ...
                            @can('view', \App\Models\License::class)
                                <li aria-hidden="true"{!! (Request::is('licenses*') ? ' class="active"' : '') !!}>
                                    <a href="{{ route('licenses.index') }}" {{$AMSSettings->shortcuts_enabled == 1 ? "accesskey=2" : ''}} tabindex="-1" data-tooltip="true" data-placement="bottom" data-title="{{ trans('general.licenses') }}">
                            ...
                            @can('index', \App\Models\Accessory::class)
                                <li aria-hidden="true"{!! (Request::is('accessories*') ? ' class="active"' : '') !!}>
                                    <a href="{{ route('accessories.index') }}" {{$AMSSettings->shortcuts_enabled == 1 ? "accesskey=3" : ''}} tabindex="-1" data-tooltip="true" data-placement="bottom" data-title="{{ trans('general.accessories') }}">
                            ...
                            @can('index', \App\Models\Consumable::class)
                                <li aria-hidden="true"{!! (Request::is('consumables*') ? ' class="active"' : '') !!}>
                                    <a href="{{ url('consumables') }}" {{$AMSSettings->shortcuts_enabled == 1 ? "accesskey=4" : ''}} tabindex="-1" data-tooltip="true" data-placement="bottom" data-title="{{ trans('general.consumables') }}">
                            ...
                            @can('view', \App\Models\Component::class)
                                <li aria-hidden="true"{!! (Request::is('components*') ? ' class="active"' : '') !!}>
                                    <a href="{{ route('components.index') }}" {{$AMSSettings->shortcuts_enabled == 1 ? "accesskey=5" : ''}} tabindex="-1" data-tooltip="true" data-placement="bottom" data-title="{{ trans('general.components') }}">
                            ...
                            @can('admin')
                                ...
                                <?php $alert_items = ($AMSSettings->show_alerts_in_menu=='1') ? Helper::checkLowInventory() : [];
                    ...
                    <ul class="sidebar-menu" data-widget="tree" {{ \App\Helpers\Helper::determineLanguageDirection() == 'rtl' ? 'style="margin-right:12px' : '' }}>
                        ...
                        @can('view', \App\Models\User::class)
                            <li id="users-sidenav-option"{!! (Request::is('users*') ? ' class="active"' : '') !!}>
                                <a href="{{ route('users.index') }}" {{$AMSSettings->shortcuts_enabled == 1 ? "accesskey=6" : ''}}>
            ...
            <footer class="main-footer hidden-print" style="display:grid;flex-direction:column;">
                ...
                <div class="1hidden-xs pull-left">
                    ...
                    <div class="pull-right">
                    @if ($AMSSettings->version_footer!='off')
                        @if (($AMSSettings->version_footer=='on') || (($AMSSettings->version_footer=='admin') && (Auth::user()->isSuperUser()=='1')))
                    ...
                    @if ($AMSSettings->support_footer!='off')
                        @if (($AMSSettings->support_footer=='on') || (($AMSSettings->support_footer=='admin') && (Auth::user()->isSuperUser()=='1')))
                            <a target="_blank" class="btn btn-default btn-xs"
                               href="https://neepco.co.in/ams-documentation"
                               rel="noopener">{{ trans('general.user_manual') }}</a>
                            <a target="_blank" class="btn btn-default btn-xs" 
                               href="mailto:it-support@neepco.co.in?subject=NEEPCO%20AMS%20Support"
                               rel="noopener">{{ trans('general.support_contact') }}</a>
                        @endif
                    @endif

                    @if ($AMSSettings->privacy_policy_link!='')
                        <a target="_blank" class="btn btn-default btn-xs" rel="noopener"
                           href="{{  $AMSSettings->privacy_policy_link }}"
                           target="_new">{{ trans('admin/settings/general.privacy_policy') }}</a>
                    @endif
                    </div>
                    <br>
                    @if ($AMSSettings->footer_text!='')
                        <div class="pull-left">
                            {!!  Helper::parseEscapedMarkedown($AMSSettings->footer_text)  !!}
                        </div>
                    @endif
                </div>
            </footer>
        ...
        @if (Auth::check())
        <script nonce="{{ csrf_token() }}">
            window.userSoundPreference = {{ Auth::user()->enable_sounds ? 'true' : 'false' }};
        </script>
        <script src="{{ url('js/sounds.js') }}" nonce="{{ csrf_token() }}"></script>
        <script src="{{ url('js/sounds-helper.js') }}" nonce="{{ csrf_token() }}"></script>
        <script nonce="{{ csrf_token() }}">
            document.addEventListener('DOMContentLoaded', function() {
                if (typeof document.soundManager !== 'undefined') {
                    document.soundManager.toggle(window.userSoundPreference);
                }
            });
        </script>
        @endif
