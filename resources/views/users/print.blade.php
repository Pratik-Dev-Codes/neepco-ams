...
@if ($AMSSettings->logo_print_assets=='1')
    @if ($AMSSettings->brand == '3')

        <h2>
            @if ($AMSSettings->acceptance_pdf_logo!='')
                <img class="print-logo" src="{{ config('app.url') }}/uploads/{{ $AMSSettings->acceptance_pdf_logo }}">
            @endif
...
@foreach ($users as $show_user)
    ...
    @if ($show_user->assets->count() > 0)
        ...
        <table
            class="ams-table table table-striped inventory"
            id="AssetsAssigned"
