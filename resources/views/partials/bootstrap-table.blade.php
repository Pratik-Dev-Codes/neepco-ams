...
<script nonce="{{ csrf_token() }}">
    $(function () {
        ...
        $('.ams-table').bootstrapTable('destroy').each(function () {
            ...
            $(this).bootstrapTable({
                ...
                pageSize: {{  (($AMSSettings->per_page!='') && ($AMSSettings->per_page > 0)) ? $AMSSettings->per_page : 20 }},
    ...
    function dateRowCheckStyle(value) {
        if ((value.days_to_next_audit) && (value.days_to_next_audit < {{ $AMSSettings->audit_warning_days ?: 0 }})) {
    ...
    $('.ams-table').on('check.bs.table .btSelectItem', function (row, $element) {
    ...
    $('.ams-table').on('check-all.bs.table', function (event, rowsAfter) {
    ...
    $('.ams-table').on('uncheck.bs.table .btSelectItem', function (row, $element) {
        var tableId =  $(this).data('id-table');
    ...
    $('.ams-table').on('uncheck.bs.table', function () {
        var buttonName =  $(this).data('bulk-button-id');
    ...
    $('.ams-table').on('uncheck-all.bs.table', function (event, rowsAfter, rowsBefore) {
    ...
    // Initialize sort-order for bulk actions (label-generation) for ams-tables
    $('.ams-table').each(function (i, table) {
    ...
    $('.ams-table').on('sort.bs.table', function (event, name, order) {
       domnode = $($(this).data('bulk-form-id')).get(0);
    ...
    function polymorphicItemFormatter(value) {
        ...
        if ((value) && (value.type)) {
            ...
            return '<nobr><a href="{{ config('app.url') }}/' + item_destination +'/' + value.id + '" data-tooltip="true" title="' + value.type + '"><i class="' + item_icon + ' text-{{ $AMSSettings->skin!='' ? $AMSSettings->skin : 'blue' }} "></i> ' + value.name + '</a></nobr>';
    ...
    function changeLogFormatter(value) {
        ...
        var pretty_index = '';
            ...
            for (var index in value) {
                ...
                if (index.startsWith('_neepco_ams')) {
                    pretty_index = index.replace("_neepco_ams", "Custom:_");
                } else {
                    pretty_index = index;
                }
    ...
    function auditImageFormatter(value){
        if (value){
            return '<a href="' + value.url + '" data-toggle="lightbox" data-type="image"><img src="' + value.url + '" style="max-height: {{ $AMSSettings->thumbnail_max_h }}px; width: auto;" class="img-responsive" alt=""></a>'
        }
    }
   ...
   function imageFormatter(value, row) {
        ...
        if (value) {
            ...
            return '<a href="' + value + '" data-toggle="lightbox" data-type="image"><img src="' + value + '" style="max-height: {{ $AMSSettings->thumbnail_max_h }}px; width: auto;" class="img-responsive" alt="' + altName + '"></a>';
        }
    }
    ...
    function labelRadioFormatter(value, row, index, field) {
        if (row) {
            return row.name == '{{ str_replace("\\", "\\\\", $AMSSettings->label2_template) }}';
        }
    }
    ...
    function cleanFloat(number) {
        ...
        if ("{{$AMSSettings->digit_separator}}" == "1.234,56") {
    ...
    function numberWithCommas(value) {
        ...
        if ((value) && ("{{$AMSSettings->digit_separator}}" == "1.234,56")){
    ...
    $(function () {
        $('#bulkEdit').click(function () {
            var selectedIds = $('.ams-table').bootstrapTable('getSelections');
