...
class SettingsController extends Controller
{
    ...
    public function postSaveFirstAdmin(SetupUserRequest $request) : RedirectResponse
    {
        ...
        $settings->default_currency = $request->input('default_currency', 'INR');
    ...
    public function postBranding(ImageUploadRequest $request) : RedirectResponse
    {
        ...
        if (!config('app.lock_passwords')) {
            ...
            $setting->site_name = $request->input('site_name', 'NEEPCO AMS');
    ...
    {
        if (! config('app.lock_passwords')) {
            Artisan::call('neecpo_ams:backup', ['--filename' => 'manual-backup-'.date('Y-m-d-H-i-s')]);
        ...
        if (! config('app.lock_passwords')) {
            ...
            if (Storage::exists($path.'/'.$filename)) {
                ...
                if ($request->input('clean')) {
                    ...
                    Artisan::call('neecpo_ams:restore', [
                ...
                Artisan::call('neecpo_ams:global-logout', ['--force' => true]);
        ...
        if (config('app.allow_purge')=='true') {
            ...
            if (!config('app.lock_passwords')) {
                ...
                if ($request->input('confirm_purge')=='DELETE') {
                    ...
                    Artisan::call('neecpo_ams:purge', ['--force' => 'true', '--no-interaction' => true]);
                    $output = Artisan::output();
