...
class CustomField extends Model
{
    ...
    {
        return '_neepco_ams'.preg_replace('/[^a-zA-Z0-9]/', '_', strtolower($name));
    }
    ...
    public function convertUnicodeDbSlug($original = null)
    {
        ...
        if (! function_exists('transliterator_transliterate')) {
            $long_slug = '_neepco_ams'.str_slug(mb_convert_encoding(trim($name),"UTF-8"), '_');
        } else {
            $long_slug = '_neepco_ams'.Utf8Slugger::slugify($name, '_');
        }
