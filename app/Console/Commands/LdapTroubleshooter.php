...
class LdapTroubleshooter extends Command
{
    ...
    public function handle()
    {
        ...
        if(!$settings->ldap_enabled) {
            $this->error("WARNING: NEEPCO AMS's LDAP setting is not turned on. (That may be OK if you're still trying to figure out settings)");
        }
        ...
        foreach($ips as $ip) {
            ...
            if(ip_in_range($ip['ip'],'10.0.0.0/8') || ip_in_range($ip['ip'],'192.168.0.0/16') || ip_in_range($ip['ip'], '172.16.0.0/12')) {
                $this->error("WARNING: Using an RFC1918 Private address for LDAP server. This may be correct, but it can be a problem if your NEEPCO AMS instance is not hosted on your private network");
            }
        }
        ...
        $this->info("STAGE 6: Test LDAP Login to NEEPCO AMS");
        foreach($ldap_urls AS $ldap_url) {
